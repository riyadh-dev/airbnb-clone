		...
		for (let index = 0; index < LISTINGS_PER_USER; index++) {
			...
			insertListingsPromises.push(
				db.insert(listings).values({
					...
					bathroomCount: faker.datatype.number({ min: 1, max: 5 }),
					bedroomCount: faker.datatype.number({ min: 1, max: 5 }),
					bedCount: faker.datatype.number({ min: 1, max: 5 }),
					guestCount: faker.datatype.number({ min: 1, max: 5 }),
